...
def user_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            try:
                # to track the cart items the user added before login
                ...
                if is_cart_item_exist:
                    ...
                    # get the cart item from user to access user product variations
                    cart_item = CartItem.objects.filter(user=user)
                    # existing variation-> db, curr variation->product_var, item_id
